...
import { VideoModelName, getModelConfig, calculateVideoCredit } from '../enums/wavespeed-models.enum';

@Injectable()
export class ImageToVideoTool implements AiToolProvider {
  private readonly logger = new Logger(ImageToVideoTool.name);
  ...
  getType(): AiToolType {
    return AiToolType.IMAGE_TO_VIDEO;
  }
  ...
  async execute(params: AiToolParams): Promise<AiToolResult> {
    ...
    if (!params.imageUrl && !params.imageFile) {
      throw new Error('需要提供源图像');
    }

    // 根据模型选择方法
    const modelName = (params.model as VideoModelName) || VideoModelName.SEEDANCE_PRO_I2V_720P;
    ...
    try {
      ...
      this.logger.error(`图片转视频失败 (${modelName}):`, error.message);
      throw error;
    }
  }
  ...
  private async generateWithWan(params: AiToolParams, modelName: VideoModelName): Promise<AiToolResult> {
    try {
      ...
      this.logger.error('Wan 2.5 生成失败:', error);
      this.logger.error('错误详情:', error.response?.data);
      throw new Error(`Wan 2.5 生成失败: ${error.response?.data?.message || error.message}`);
    }
  }
  ...
  private async generateWithVeo(params: AiToolParams, modelName: VideoModelName): Promise<AiToolResult> {
    try {
      ...
      if (params.lastFrame) {
        requestData.last_image = params.lastFrame;
      }
